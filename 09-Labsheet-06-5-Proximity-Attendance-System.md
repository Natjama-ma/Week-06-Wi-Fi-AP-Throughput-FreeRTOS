...
~~~
การใช้ SoftAP ทำให้ระบบ Web Server รันอยู่บนเครือข่าย Local Network นักศึกษาจึงไม่สามารถเข้าเว็บเพื่อเช็กชื่อผ่านอินเทอร์เน็ตจากข้างนอกได้ และเมื่อนำมาผสานกับเกณฑ์ RSSI จะเป็นการยืนยันตัวตนเชิงพื้นที่ (Spatial Authentication)
ที่บังคับว่าอุปกรณ์ทางกายภาพจะต้องตั้งอยู่ใกล้กับบอร์ด ESP32 จริงๆ ทำให้การฝากเพื่อนเช็กชื่อทำได้ยากมาก เพราะนักศึกษาจะต้องฝากสมาร์ตโฟนส่วนตัวเครื่องหลักของตนเองมาที่ห้องเรียนด้วย

~~~
2. เหตุใดระดับเกณฑ์ RSSI ที่ `-55 dBm` จึงเหมาะสมสำหรับการระบุตำแหน่งอุปกรณ์ให้อยู่ภายในรัศมีโต๊ะปฏิบัติการ?
~~~
ค่าความแรง -55 dBm เป็นระดับสัญญาณที่ค่อนข้างแรง ทั่วไปจะเทียบเท่ากับระยะห่างประมาณ 1-3 เมตร (Line of Sight) หากนักศึกษาอยู่นอกห้องเรียน หรือมีสิ่งกีดขวางเช่นกำแพงห้อง สัญญาณจะเกิดการลดทอน (Attenuation)
จนตกไปอยู่ในช่วง -70 dBm หรือต่ำกว่านั้น การตั้งเกณฑ์ไว้ที่ -55 dBm จึงเป็นการสร้างกรอบพื้นที่เสมือน (Geofencing) เพื่อบีบรัศมีให้อยู่แค่บริเวณโต๊ะปฏิบัติการได้อย่างแม่นยำ
~~~
...
~~~
จะต้องปรับปรุงสถาปัตยกรรมระบบเพิ่มเติม 2 ส่วน
1. ปรับโหมด Wi-Fi เป็น APSTA (Access Point + Station): เพื่อให้บอร์ด ESP32 สามารถกระจายสัญญาณให้นักศึกษาเชื่อมต่อเพื่อเช็กชื่อ (AP) และในขณะเดียวกันก็สามารถเชื่อมต่อเครือข่าย Wi-Fi ของสถาบันเพื่อออกอินเทอร์เน็ตได้ (STA)
2. เพิ่มคอมโพเนนต์ HTTP Client (esp_http_client): เพื่อให้ ESP32 ทำหน้าที่เป็นฝั่ง Client ในการสร้าง REST API (เช่น POST Request) นำชุดข้อมูล JSON ที่ประกอบด้วย MAC Address, ค่า RSSI และ Timestamp
ส่งขึ้นไปบันทึกลงบนฐานข้อมูล Cloud Database หรือ Google Apps Script ได้โดยตรง
~~~
