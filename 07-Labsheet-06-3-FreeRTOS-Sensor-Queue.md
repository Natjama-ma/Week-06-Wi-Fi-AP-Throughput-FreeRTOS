...
| :--- | :---: | :---: | :---: |
| **`SensorCollectorTask`** | 3072 |2,080 Words |ปลอดภัย |
| **`NetworkCommTask`** | 4096 |3,100 Words  |ปลอดภัย |
...
~~~
การใช้ FreeRTOS Queue มีความปลอดภัยแบบ Thread-Safe เนื่องจากภายใน Queue มีกลไกจัดการการเข้าถึงทรัพยากร (เช่น การจำกัดสิทธิด้วย Mutex หรือ Critical Section) ช่วยป้องกันปัญหา Race Condition
ที่เกิดจากการที่หลาย Task พยายามอ่านและเขียนข้อมูลพร้อมกัน ต่างจากการใช้ตัวแปร Global ทั่วไปที่หาก OS สลับการทำงาน (Context Switch) ในขณะที่ Task หนึ่งเขียนข้อมูลยังไม่เสร็จ แล้วอีก Task เข้ามาอ่าน
ก็จะทำให้ได้ข้อมูลที่ผิดพลาดหรือขยะ (Corrupted Data) ไปประมวลผลต่อ
~~~
2. ค่า **Stack High Water Mark** มีประโยชน์อย่างไรในการตรวจวินิจฉัยปัญหาบั๊กในระบบเรียลไทม์ (RTOS)?
~~~
จะบอกถึง "พื้นที่หน่วยความจำสแตกที่เหลือน้อยที่สุด" (Minimum Remaining Stack Space) นับตั้งแต่ Task นั้นถูกสร้างขึ้นมา ประโยชน์หลักคือใช้ประเมินสุขภาพของสแตก หากค่านี้ลดลงจนเข้าใกล้ศูนย์ (0) แสดงว่า Task
มีความเสี่ยงสูงมากที่จะเกิดภาวะ Stack Overflow ทำให้ไมโครคอนโทรลเลอร์แครชหรือรีบูตตัวเอง จึงใช้ค่านี้วิเคราะห์เพื่อจูนขนาดสแตกใน xTaskCreate ให้มีขนาดพอดี ไม่ใหญ่เกินไปจนเปลือง RAM และไม่น้อยเกินไปจนระบบพัง
~~~
...
~~~
จะmeให้พื้นที่ใน Queue เต็มอย่างรวดเร็ว (Queue Full) เมื่อคิวเต็ม การเรียกใช้ฟังก์ชัน xQueueSend() ใน vSensorTask จะไม่สามารถนำข้อมูลใหม่ใส่ลงคิวได้ และจะคืนค่าสถานะความล้มเหลว (Failed / errQUEUE_FULL) ออกมา
ระบบจะรับมือโดยการทิ้งข้อมูลเซนเซอร์ชุดใหม่สุดไป จนกว่า vNetworkTask จะดึงข้อมูลเก่าออกจากคิวไปประมวลผลจนเกิดพื้นที่ว่าง ป้องกันไม่ให้ระบบโดยรวมเกิดการบล็อกหรือค้าง (Deadlock)
~~~
